...
Each computational domain is decomposed with 128 processors. The specific computational cost of each LES is between 2.07 $\mu s/s$ and 8.31 $\mu s/s$ and varies with the combination of models used. For each LES, statistics were collected over a period in the range 277 ms to 1216 ms, with an average of 605 ms. The total computational cost of each simulation ranged from approximately 86,000 cpu-hours to 235,000 cpu-hours, for a cumulative total of 1.8 million cpu-hours over all simulations. %The latter and other simulation details are presented in \cref{tab:lesresults:comput:details}. NEED TO FINISH THIS TABLE.
