...
When these filtering operations are applied to the conservation equations for mass, momentum, and energy or scalar transport equations, subfilter contributions to the chemical source term as well as molecular and turbulent transport terms arise. For instance, the filtered transport equation for a scalar $\phi$ is given by
\begin{equation}\label{eq:subfilter:scalar}
  \ftrder{\tf{\phi}} = \pder[]{x_j}\left( \mean{\rho}\tf{D\pder[\phi]{x_j}} \right) + \dsff[\tf{\phi}]{\tf{u_j \phi}} + \fst[m]{\phi},
\end{equation}
where the first term on the right-hand side contains subfilter contributions to molecular diffusion fluxes, the second term is known as the subfilter scalar flux and contains the effects of turbulent transport, and the third term is the filtered source term. These unclosed terms represent interactions between subfilter-scale quantities and must be modeled. 
...
\end{equation}
where $\tf{P}(\xi)$ includes a beta distribution for the mixture fraction and $P(\mc{M}_j|\xi)$ is unknown. 
